...
import { updateUserLiveClass, updateImage } from "../../dataServices/Services";
...
const EditLiveClass = ({ liveClasses }) => {
  ...
  const getSingleLiveClass = () => {
    ...
    if (singleLiveClass[0].recur !== "") {
      setRepeatValue(["repeat"]);
    }
    console.log(singleLiveClass[0]);
    if (singleLiveClass[0].type === "Virtual") {
    } else {
      let listloc = [...locationsList];
      let newListLoc = {
        key: singleLiveClass[0].location._id,
        value: singleLiveClass[0].location.name
      };
      listloc.push(newListLoc);
      setLocationsList(listloc);
      setSelectedLocation(singleLiveClass[0].location);
    }
    setCoverImage(singleLiveClass[0].coverImage);
    setLiveType(singleLiveClass[0].type);
    setLength(singleLiveClass[0].maxLength);
    setTrainer(singleLiveClass[0].trainer);
    setDate(singleLiveClass[0].date);
    let fitType = [...selectedType];
    fitType.push(singleLiveClass[0].fitnessType);
    setSelectedType(fitType);
    let tarea = [...selectedTargetArea];
    tarea.push(singleLiveClass[0].targetArea);
    setSelectedTargetArea(tarea);
    let fgoal = [...selectedFitnessGoal];
    fgoal.push(singleLiveClass[0].fitnessGoal);
  ...
  return (
    <Formik
      initialValues={{
        ...
        link: liveClass && liveClass.link,
        recur: liveClass && liveClass.recur,
        startTime: liveClass && liveClass.startTime,

        capacity: liveClass && liveClass.capacity
      }}
