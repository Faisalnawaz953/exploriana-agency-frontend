...
function Inbox({ user, subUser, chatRooms, updateChatRooms }) {
  const [selectedChatRoom, setSelectedChatRoom] = React.useState(null)
  const [messageText, setMessageText] = React.useState("")
  const [loading, setLoading] = React.useState(false)
  let loggedUser = localStorage.getItem("userType") === "user" ? subUser : user
  ...
  const getChatRooms = async () => {
    setLoading(true)
    const res = await getAllChatRoomsByUserId(loggedUser?._id)
    if (res.success) {
      setLoading(false)
  ...
  const getTotalUnreadMessagesCount = (messages) => {
    const unread = messages.filter(
      (mess) => mess.seen === false && mess.senderId !== loggedUser?._id
    )
    if (messages[messages.length - 1].senderId !== loggedUser?._id) {
  ...
  const showUserImage = (participants) => {
    let part = participants.filter((part) => part._id !== loggedUser?._id)
              ...
              {!isEmpty(chatRooms) &&
                chatRooms.map((chatRoom, i) => (
                  <Conversation
                    ...
                    name={
                      chatRoom.participants[0]._id === loggedUser?._id
            ...
            {selectedChatRoom ? (
              <ChatContainer>
                <ConversationHeader>
                  ...
                  <Avatar
                    src={
                      selectedChatRoom.participants[0]._id === loggedUser?._id
